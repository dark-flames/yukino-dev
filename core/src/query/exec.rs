...
pub trait FetchOne<T: Value, TTags: TagList>: Executable<T, TTags, ResultType=SingleRow> {
    async fn exec<'c, 'e, DB: Database, E: 'e + Executor<'e, Database=DB>>(self, executor: E) -> Result<T, Error>
        where Self: Sized,
        ...
        let result: GenericArray<DatabaseValue, ValueCountOf<T>> = query_as_with(&query, args)
...
#[async_trait]
pub trait FetchMulti<T: Value, TTags: TagList>: Executable<T, TTags, ResultType=MultiRows> {
    async fn exec<'c, 'e, DB: Database, E: 'e + Executor<'e, Database=DB>>(self, executor: E) -> Result<Vec<T>, Error>
        where Self: Sized,
              DatabaseValue: for<'q> AppendToArgs<'q, DB>,
              for<'q> <DB as  HasArguments<'q>>::Arguments: IntoArguments<'q, DB>,
              <ValueCountOf<T> as ArrayLength<DatabaseValue>>::ArrayType: Unpin,
        ...
        let query = state.to_string();
        let args = state.args::<DB>();
        let result: Vec<GenericArray<DatabaseValue, ValueCountOf<T>>> = query_as_with(&query, args)
