...
>: EntityWithView + Association<Parent, ForeignKeyType=ForeignKey>
    ...
    fn belonging_to_query(r: QueryResultFilter<Parent>) -> QueryResultFilter<Self> where Self: Sized {
        r.build_query()
    }
}
...
pub trait BelongsToView<
    ...
    Parent: EntityWithView + WithPrimaryKey<Type=ForeignKey>,
    ForeignKey: Value
> where QueryResultFilter<Parent>: AssociationBuilder<Children, Parent, ForeignKey>,
      Parent::View: ViewWithPrimaryKey<Type=ForeignKey>
{
    fn belonging_to_view(r: &Parent::View) -> QueryResultFilter<Children> where Self: Sized {
