...
use query_builder::{DatabaseValue, Expr, FunctionCall, SelectQuery, SubqueryFunction, SubqueryFunctionCall};
...
pub trait SubqueryView<T: Value<L=U1>> {
    ...
    fn any(self) -> SubqueryFnCallView<T> where Self: Sized {
        SubqueryFnCallView::create(self.subquery(), SubqueryFunction::Any)
    }

    fn all(self) -> SubqueryFnCallView<T> where Self: Sized {
...
impl<T: Value<L=U1>> SubqueryFnCallView<T> {
    pub fn create(subquery: SelectQuery, func: SubqueryFunction) -> Self {
        Self { fn_call: SubqueryFunctionCall {
            function: func,
            subquery
        }, _ty: PhantomData }
    }
